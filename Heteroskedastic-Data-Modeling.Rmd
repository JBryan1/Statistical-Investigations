...
TALK ABOUT FALSE POSITIVES OF GQ TEST AS P -> N and when MC present.
...
GQ.test = rep(NA,9)
GQ.test[1] = round(gqtest(lm_1)[[5]],3)
GQ.test[2] = round(gqtest(lm_2)[[5]],3)
GQ.test[3] = round(gqtest(lm_10)[[5]],3)
GQ.test[4] = round(gqtest(lm_100)[[5]],3)
GQ.test[5] = NA #breaks down at p=250
GQ.test[6] = round(gqtest(lm_2mc)[[5]],3)
GQ.test[7] = round(gqtest(lm_10mc)[[5]],3)
GQ.test[8] = round(gqtest(lm_100mc)[[5]],3)
GQ.test[9] = NA #breaks down at p=250
...
HMC.test = rep(NA,9)
HMC.test[1] = round(hmctest(lm_1)[[3]],3)
HMC.test[2] = round(hmctest(lm_2)[[3]],3)
HMC.test[3] = round(hmctest(lm_10)[[3]],3)
HMC.test[4] = round(hmctest(lm_100)[[3]],3)
HMC.test[5] = NA #breaks down at p=250
HMC.test[6] = round(hmctest(lm_2mc)[[3]],3)
HMC.test[7] = round(hmctest(lm_10mc)[[3]],3)
HMC.test[8] = round(hmctest(lm_100mc)[[3]],3)
HMC.test[9] = NA #breaks down at p=250
...
stat.table = matrix(data = c(GQ.test,HMC.test, BP.test),
                        byrow = TRUE,
                        ncol = 9,
                        nrow = 3)
stat.table = data.frame(stat.table,
                        row.names = c("Goldfeld-Quandt",
                                      "Harrison-McCabe",
                                      "Breusch-Pagan"))
colnames(stat.table) = c(1,2,10,100,250, "2 MC", "10 MC", "100 MC", "250 MC")
knitr::kable(stat.table, caption = "Table 1. Statistical Tests for Heteroskedasticity")
```

*Harrison-McCabe test*

The Harrison-McCabe test is similar in that an arbitrary fractional breakpoint is first selected. The test statistic then tests the divergence of the ratio of the residual sum of squares for each half of the residuals from the selected fractional breakpoint. The null hypothesis $H_0: b = \frac{RSS_{<b}}{RSS}$ is asymptotically bounded by Beta distributions that are a function of $n$ , $p$ and the number of observations below the breakpoint. 
